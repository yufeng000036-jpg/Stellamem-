...
请通过私密渠道联系我们（安全邮箱：**2024981488@qq.com**），并提供：
...
我们会在 **3 个工作日**内确认收到，并在修复后公开披露。感谢你以负责任的方式报告。
...
- **fail-soft：注入失败时返回空、Agent 继续运行，不因记忆层故障而阻断对话**；
- 有单文件注入字符上限与敏感档案摘要策略（可用 plugin config 调整）；
- 有超时、有日志、可禁用。
